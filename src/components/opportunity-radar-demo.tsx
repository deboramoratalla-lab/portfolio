...
type Opportunity = { id: string; title: string; company: string; location: string; type: string; category: string; source: "Jobicy" | "Remotive" | "Remote OK" | "Ashby" | "Glassdoor" | "Himalayas" | "Arbeitnow"; origin: "Public feed" | "Direct company feed" | "Commercial job API"; url: string; publishedAt: string | null }
...
export function OpportunityRadarCover() {
  return <div className="opportunity-radar-cover" aria-hidden="true">
    ...
    <footer><span>Sources</span><b>7 feeds / live links</b></footer>
  </div>
}
...
export function OpportunityRadarDemo() {
  ...
  return <section className="opportunity-radar" id="opportunity-radar" aria-labelledby="radar-title">
    ...
    <div className="opportunity-radar-surface">
      ...
      <div className="radar-controls"><label><IconSearch size={17} /><span className="sr-only">Search opportunities</span><input value={query} onChange={event => { setQuery(event.target.value); setShown(12) }} placeholder="Search title, company or location" /></label><div className="radar-filter-group"><span>Role family</span><div className="radar-category-list" aria-label="Filter by role family">{categories.map(item => <button type="button" className={category === item ? "is-selected" : ""} onClick={() => { setCategory(item); setShown(12) }} key={item}>{item}</button>)}</div></div><div className="radar-filter-group"><span>Source type</span><div className="radar-category-list" aria-label="Filter by source type">{(["All sources", "Direct company feed", "Commercial job API", "Public feed"] as const).map(item => <button type="button" className={origin === item ? "is-selected" : ""} onClick={() => { setOrigin(item); setShown(12) }} key={item}>{item}</button>)}</div></div></div>
      ...
      {!isLoading && !error && matches.length > visible.length && <div className="radar-load-more"><span>Showing {visible.length} of {matches.length}</span><button type="button" onClick={() => setShown(current => current + 12)}>Show 12 more</button></div>}
