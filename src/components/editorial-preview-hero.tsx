...
export function EditorialPreviewHero() {
  ...
  return <section ref={ref} className="editorial-hero" aria-label="Introduction" onPointerMove={paintPixels} onPointerLeave={() => { lastCell.current = -1 }}>
    ...
    <div className="editorial-hero-top">
      ...
      <motion.span initial={reduced ? false : { opacity: 0 }} animate={{ opacity: 1 }} transition={{ ...enter, delay: .35 }}>Product designer → Design engineer</motion.span>
    </div>

    <div className="editorial-hero-stage">
      <motion.h1 className="editorial-title" style={{ y: titleY }}>
        <motion.span initial={reduced ? false : { opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ ...enter, delay: .18 }}>I design</motion.span>
      ...
      <motion.aside className="editorial-proof" style={{ y: panelY }} initial={reduced ? false : { opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ ...enter, delay: .58 }}>
        <div className="editorial-proof-meta"><span>8+ years</span><i/><span>B2B + enterprise</span><i/><span>Systems + AI</span></div>
        <p>I make roles, states and product decisions visible before they become interface debt — working closely with engineering from first model to shipped product.</p>
