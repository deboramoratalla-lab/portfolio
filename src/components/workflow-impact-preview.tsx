  ...
  return <section className={styles.root} id="workflow-impact">
    ...
    <div className={styles.app}>
      ...
      <div className={styles.workspace}>
        <aside className={styles.sidebar}><div className={styles.sidebarHead}><span>Example changes</span><small>Choose one</small></div><div className={styles.scenarios}>{(Object.keys(states) as Version[]).map(key => <button className={version === key ? styles.selected : ""} key={key} onClick={() => { setVersion(key); setResult(null); setActed(false); setShowCases(false); setComparingCurrent(false) }}><strong>{states[key].label}</strong><small>{states[key].note}</small></button>)}</div><footer><IconShieldCheck size={15} /><span>Replay mode</span></footer></aside>
        <main className={styles.canvas}>
          <section className={styles.decision}><div className={styles.impactVisual}><span>Can this change be published?</span><strong>{state.impact}</strong><small>across 3 representative paths</small></div><div><h3>{state.recommendation}</h3><p>{state.copy}</p><div className={styles.decisionReasons} aria-label="Why this recommendation"><span>{state.reasons[0]}</span><span>{state.reasons[1]}</span></div>{result && <small className={styles.liveDecision}>{result.decision}</small>}</div><div className={styles.outcomeDelta} aria-label="Representative outcome changes">{state.rows.map(row => <div key={row[0]}><small>{row[0]}</small><span>{row[1]}</span><IconArrowRight size={13} /><b>{row[2]}</b></div>)}</div><button className={styles.run} onClick={result ? () => setActed(true) : runReplay} disabled={running || acted}>{running ? "Running replay" : acted ? <><IconCheck size={16} /> {state.confirmation}</> : result ? <><IconCheck size={16} /> {state.action}</> : <><IconPlayerPlay size={16} /> Run replay</>}</button></section>
          <details className={styles.caseDetails} open={showCases}><summary onClick={(event) => { event.preventDefault(); setShowCases(value => !value) }}><span>{showCases ? "Hide changed cases" : "View changed cases"}</span><small>Compare every sampled route</small></summary><section className={styles.replay}><header><div><IconFileDiff size={16} /><span>Historical comparison</span></div><button className={comparingCurrent ? styles.comparing : ""} onClick={() => setComparingCurrent(value => !value)}><IconSparkles size={15} /> {comparingCurrent ? "Show proposed" : "Compare to current"}</button></header>{state.rows.map(row => <article key={row[0]}><strong>{row[0]}</strong><span>{row[1]}</span><IconArrowRight size={14} /><span>{comparingCurrent ? row[1] : row[2]}</span><b>{comparingCurrent ? "CURRENT" : row[3]}</b></article>)}</section></details>
