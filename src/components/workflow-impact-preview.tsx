    ...
    <div className={styles.releaseApp}>
      ...
      <div className={styles.releaseWorkspace}>
        <main className={styles.discoveryArea}>
          ...
          {viewMode==="graph"?<div className={styles.changeGraph} aria-label="Agent change graph"><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><path d="M37 21 H64"/><path d="M37 51 H50 C57 51 57 21 64 21"/><path d="M37 51 H64"/><path d="M37 51 H50 C57 51 57 81 64 81"/></svg><article className={`${styles.graphEntity} ${styles.agentEntity}`}><span><IconWebhook size={18}/></span><div><b>support-agent</b><small>Published workflow</small></div></article><article className={`${styles.graphEntity} ${styles.historyEntity}`}><span><IconClock size={18}/></span><div><b>Replay sample</b><small>3 conversation traces</small></div><i>3</i></article>{(Object.keys(states) as Version[]).map((key,index)=><button type="button" key={key} aria-pressed={version===key} className={`${styles.graphEntity} ${styles.changeEntity} ${styles[`changeEntity${index+1}`]}`} onClick={()=>{setVersion(key);setResult(null);setActed(false)}}><span><IconHierarchy size={18}/></span><div><b>{states[key].label}</b><small>{states[key].path}</small></div>{key==="changed"&&<i>2</i>}</button>)}</div>:<div className={styles.changeTable}><header><span>Change</span><span>Configuration</span><span>Impact</span><span>Status</span></header>{(Object.keys(states) as Version[]).map(key=><button type="button" key={key} aria-pressed={version===key} onClick={()=>{setVersion(key);setResult(null);setActed(false)}}><b>{states[key].label}</b><span>{states[key].path}</span><span>{states[key].impact}</span><em>{key==="safe"?"Ready":key==="changed"?"Review":"Blocked"}</em></button>)}</div>}
        </main>
