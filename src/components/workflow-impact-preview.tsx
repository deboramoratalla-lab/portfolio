...
export function WorkflowImpactPreview() {
  ...
  const changedCount = version === "safe" ? 0 : 3
  ...
  return <section className={styles.root} id="workflow-impact">
    ...
    <div className={styles.app}>
      ...
      <div className={styles.workspace}>
        <aside className={styles.sidebar}><div className={styles.sidebarHead}><span>Test a change</span><small>3 scenarios</small></div><div className={styles.scenarios}>{(Object.keys(states) as Version[]).map((key, index) => <button type="button" aria-pressed={version === key} className={version === key ? styles.selected : ""} key={key} onClick={() => { setVersion(key); setResult(null); setActed(false); setComparingCurrent(false) }}><span className={styles.scenarioIndex}>0{index + 1}</span><span><strong>{states[key].label}</strong><small>{states[key].note}</small></span><IconChevronRight size={16} aria-hidden="true" /></button>)}</div><footer><IconShieldCheck size={16} aria-hidden="true" /><span><b>Safe preview</b><small>No production writes</small></span></footer></aside>
        <div className={styles.canvas}>
          <section className={`${styles.decision} ${styles[version]}`}>
            <div className={styles.decisionLead}><span>Publish readiness</span><div className={styles.risk}><i />{riskLabel}</div><h3>{state.recommendation}</h3><p>{state.copy}</p><div className={styles.decisionActions}><button type="button" className={styles.run} onClick={result ? () => setActed(true) : runReplay} disabled={running || acted}>{running ? "Running replay…" : acted ? <><IconCheck size={17} /> Done</> : result ? <><IconCheck size={17} /> {state.action}</> : <><IconPlayerPlay size={17} /> Run replay</>}</button><small>3 synthetic conversations · no writes</small></div>{result && <small className={styles.liveDecision} role="status" aria-live="polite">{result.decision}</small>}</div>
            <div className={styles.impactPanel}><header><div><span>Replay impact</span><strong>{state.impact}</strong></div><small>{changedCount}/3 routes changed</small></header><div className={styles.impactChart} aria-label={`${changedCount} of 3 representative routes changed`}>{state.rows.map((row, index) => <div key={row[0]} className={row[3] === "MATCH" ? styles.chartMatch : row[3] === "FAILS" ? styles.chartFail : styles.chartChange}><span style={{height: `${38 + index * 22}%`}} /><small>{index + 1}</small></div>)}</div><div className={styles.chartLegend}><span><i />Unchanged</span><span><i />Changed</span><span><i />Failed</span></div></div>
          </section>
