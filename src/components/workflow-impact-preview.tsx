...
type ApprovalReceipt = { requestId: string; executionId: string; status: "pending"; createdAt: string; persistence: string; externalWrite: false }
    ...
    <div className={styles.reviewApp}>
      ...
      <div className={styles.reviewBody}>
        ...
        <main className={`${styles.reviewDetail} ${styles[version]}`}>
          ...
          <div id="replay-status" className={styles.liveStatus} role="status" aria-live="polite">{running ? "Sending the selected fixture through the n8n webhook." : error ? error : approval ? `Approval ${approval.requestId} is pending in the n8n Data Table. No CRM write was made.` : result ? `Receipt ${result.traceId} received from n8n.` : "Ready for a synthetic replay."}</div>
