...
export function WorkflowImpactPreview() {
  ...
  return <section className={styles.experiment} id="workflow-impact">
    ...
    <div className={styles.releaseApp}>
      <nav className={styles.primaryNav} aria-label="Product sections"><span className={styles.brandGlyph}><IconHierarchy size={18}/></span><span>Agents</span><span>Replays</span><span>Changes</span><strong>Discovery</strong><span>Policies</span><span>Workflows</span></nav>
      <header className={styles.contextBar}><span className={styles.backMark}>‹</span><div><IconFileDiff size={17}/><span>Draft 18</span><b>{state.label}</b></div><button type="button" onClick={runReplay} disabled={running}><IconPlayerPlay size={14}/>{running?"Running replay":"Run replay"}</button></header>
      <div className={styles.releaseWorkspace}>
        <main className={styles.discoveryArea}>
          ...
          {viewMode==="graph"?<div className={styles.changeGraph} aria-label="Agent change graph"><svg viewBox="0 0 900 620" preserveAspectRatio="none" aria-hidden="true"><path d="M390 155 C460 155 470 250 530 275"/><path d="M335 350 C430 350 450 305 530 285"/><path d="M335 350 C390 420 430 510 520 510"/></svg><article className={`${styles.graphEntity} ${styles.agentEntity}`}><span><IconWebhook size={20}/></span><div><b>support-agent</b><small>Published workflow</small></div></article><article className={`${styles.graphEntity} ${styles.historyEntity}`}><span><IconClock size={20}/></span><div><b>Replay sample</b><small>3 conversation traces</small></div><i>3</i></article>{(Object.keys(states) as Version[]).map((key,index)=><button type="button" key={key} aria-pressed={version===key} className={`${styles.graphEntity} ${styles.changeEntity} ${styles[`changeEntity${index+1}`]}`} onClick={()=>{setVersion(key);setResult(null);setActed(false)}}><span><IconHierarchy size={20}/></span><div><b>{states[key].label}</b><small>{states[key].path}</small></div>{key==="changed"&&<i>2</i>}</button>)}</div>:<div className={styles.changeTable}><header><span>Change</span><span>Configuration</span><span>Impact</span><span>Status</span></header>{(Object.keys(states) as Version[]).map(key=><button type="button" key={key} aria-pressed={version===key} onClick={()=>{setVersion(key);setResult(null);setActed(false)}}><b>{states[key].label}</b><span>{states[key].path}</span><span>{states[key].impact}</span><em>{key==="safe"?"Ready":key==="changed"?"Review":"Blocked"}</em></button>)}</div>}
        </main>
        <aside className={`${styles.detailPanel} ${styles[version]}`}><section className={styles.detailTitle}><span className={styles.detailIcon}><IconHierarchy size={21}/></span><div><h3>{state.label}</h3><p>{state.path}</p></div></section><div className={styles.riskFacts}><div><small>Status</small><strong><i/>{riskLabel}</strong></div><div><small>Impact</small><b>{state.impact}</b></div><div><small>Source</small><b>n8n replay</b></div></div><div className={styles.detailTabs}><strong>Overview</strong><span>Activity log</span></div><section className={styles.insightSection}><h4>Insights</h4><div><span>{state.reasons[0]}</span><span>{state.reasons[1]}</span><span>Production writes disabled</span><span>3 traces replayed</span></div></section><section className={styles.routeSection}><h4>Changed routes</h4>{state.rows.map(row=><article key={row[0]}><div><b>{row[0]}</b><small>{row[1]}</small></div><IconArrowRight size={14}/><strong>{row[2]}</strong></article>)}</section><section className={styles.summarySection}><h4>Review summary</h4><p>{state.copy}</p></section><dl className={styles.propertyList}><div><dt>Workflow</dt><dd>support-agent</dd></div><div><dt>Environment</dt><dd>production-shadow</dd></div><div><dt>Transport</dt><dd>n8n webhook</dd></div><div><dt>Execution</dt><dd>{result?.proxyLatencyMs?`${result.proxyLatencyMs} ms`:"Not run"}</dd></div></dl>{result&&<p className={styles.resultNotice} role="status">{result.decision}</p>}<footer><button type="button" onClick={result?()=>setActed(true):runReplay} disabled={running||acted}>{running?"Running replay":acted?<><IconCheck size={15}/>Decision saved</>:result?state.action:<><IconPlayerPlay size={15}/>Run selected change</>}</button><small>Read-only replay · no external actions</small></footer></aside>
      </div>
    </div>
  </section>
}
