    ...
    <div className={styles.app}>
      ...
      <div className={styles.workspace}>
        ...
        <main className={styles.canvas}>
          <section className={styles.decision}><div><span>Can this change be published?</span><strong>{state.impact}</strong></div><div><h3>{state.recommendation}</h3><p>{state.copy}</p>{result && <small className={styles.liveDecision}>{result.decision}</small>}</div><button className={styles.run} onClick={result ? () => setActed(true) : runReplay} disabled={running || acted}>{running ? "Running replay" : acted ? <><IconCheck size={16} /> {state.confirmation}</> : result ? <><IconCheck size={16} /> {state.action}</> : <><IconPlayerPlay size={16} /> Run replay</>}</button></section>
