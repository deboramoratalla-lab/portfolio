    ...
    <div className={styles.reviewApp}>
      ...
      <div className={styles.reviewBody}>
        <aside className={styles.changeList}>
          <header><span>Replay setup</span><h3>Scenario to test</h3><p>Select one change to run against the governed agent.</p></header>
          <div className={styles.scenarioOptions} role="radiogroup" aria-label="Scenario to replay">{(Object.keys(states) as Version[]).map((key) => <button className={styles[key]} type="button" role="radio" key={key} aria-checked={version === key} onClick={() => { setVersion(key); setResult(null); setActed(false) }}><span className={styles.radioMark} aria-hidden="true"><i /></span><span className={styles.changeSymbol}><IconFileDiff size={17} /></span><span className={styles.scenarioCopy}><strong>{states[key].label}</strong><small>{states[key].path}</small></span><span className={styles.changeImpact}><strong>{states[key].impact}</strong><small>{version === key ? "Selected" : "Select"}</small></span></button>)}</div>
        ...
        <main className={`${styles.reviewDetail} ${styles[version]}`}>
          ...
          {panel === "architecture" ? <section className={styles.architecturePanel} aria-label="Published n8n agent architecture">
            <div className={`${styles.phase} ${styles.understandPhase}`}><header><span>Understand</span><b>Classify before reasoning</b></header><div className={styles.node}><IconWebhook size={18} /><span><b>Chat Trigger</b><small>Support request</small></span></div><div className={styles.node}><IconSearch size={18} /><span><b>Classify request</b><small>Intent and risk</small></span></div></div>
            ...
            <div className={`${styles.phase} ${styles.observePhase}`}><header><span>Observe</span><b>Make it auditable</b></header><div className={styles.node}><IconReceipt size={18} /><span><b>Execution receipt</b><small>Decision trace</small></span></div><div className={styles.node}><IconCheck size={18} /><span><b>Safety summary</b><small>User response</small></span></div></div>
