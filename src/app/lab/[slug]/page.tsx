...
const articles: Record<string, ArticleContent> = {
  "gpu-job-triage": {
    lead: "I designed a decision model for a familiar AI infrastructure moment, then built a real telemetry path to test how signals should arrive, be checked and be explained. The batch scenario is a model, not a claim about a live cloud operation.",
    quote: "The value of observability is not the chart. It is the moment when a person can act with enough context.",
    sections: [
      { title: "Model the decision, not the cluster", paragraphs: ["The batch and its cost exposure are a clearly labelled decision scenario. They are not presented as live Nebius data or as a real GPU workload. The scenario lets me test the product problem: is it obvious when more capacity would waste money, and when pausing is the responsible choice?", "The first reading is utilisation. The second is cost at risk. Time at risk and decision confidence provide the supporting context. The interface keeps the recommendation close to the reasoning that supports it."] },
      { title: "Use live signals to verify the evidence path", paragraphs: ["The Signal health panel is real. A small local Node process samples this Mac every 15 seconds and sends only aggregate readings to Grafana Cloud using Prometheus remote write. A private Vercel route queries the latest series and the browser polls that route.", "That panel exposes the path from local emitter to remote write, Grafana Cloud and the public dashboard. It gives each layer a state, cadence and boundary, so the live label has evidence behind it without pretending that the decision scenario is operational data."] },
      { title: "Keep the product surface smaller than the tooling", paragraphs: ["Grafana stores and queries the time series. It is not copied into the portfolio as a dense admin screen. The prototype uses a smaller surface to make the decision model understandable, then makes the source and freshness of its real evidence inspectable.", "A public reader can see selected aggregate host signals. They cannot see a hostname, IP address, running processes, credentials or the Grafana workspace. The technical stack remains inspectable in the repository, while Grafana remains the place for deeper analysis."] },
      { title: "How I worked", paragraphs: ["This is a personal portfolio experiment. I set the product framing and visual direction, then iterated the implementation with Codex. I verified the integration by checking the live Grafana query, the Vercel API response and the active local emitter before describing any signal as live.", "The current scope is intentionally narrow. It combines a conceptual capacity decision with a real telemetry loop and privacy boundary. It does not claim access to a production GPU platform, Nebius customer data or operational incident response."] },
    ],
    close: "The experiment holds two ideas together: a useful decision model can make the cost of waiting visible, and a trustworthy product must also say which signals are real, where they came from and what has been left out.",
  },
