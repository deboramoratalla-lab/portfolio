...
const articles: Record<string, ArticleContent> = {
  "gpu-job-triage": {
    lead: "I built a small observability experiment to test a product question: can a public surface show a changing system clearly without pretending to be the system itself?",
    quote: "A metric becomes useful when its source, boundary and freshness are easy to inspect.",
    sections: [
      { title: "Start with a boundary, not a dashboard", paragraphs: ["I did not want to publish an invented GPU workload. This Mac is not a cluster, so the experiment now says exactly what it is: a selected view of one local machine's telemetry.", "The public page exposes CPU load, memory used, active network interfaces and host uptime. It does not expose a hostname, IP address, running processes, credentials or the Grafana workspace."] },
      { title: "Make the path inspectable", paragraphs: ["A small local Node process samples the Mac every 15 seconds. It sends only those aggregate readings to Grafana Cloud using Prometheus remote write. A private Vercel route queries the latest series and the browser polls that route.", "The Signal health panel follows the same path: local emitter, remote write, Grafana Cloud and the public dashboard. It gives each layer a state, cadence and boundary, so the live label has evidence behind it."] },
      { title: "Keep the product surface smaller than the tooling", paragraphs: ["Grafana stores and queries the time series. It is not copied into the portfolio as a dense admin screen. The prototype uses a smaller surface to make freshness, source and selected readings legible at a glance.", "That choice leaves a useful path for further investigation. The public view gives context, while the technical stack remains inspectable in the repository and Grafana remains the place for deeper analysis."] },
    ...
    close: "The useful outcome is not a bigger dashboard. It is a small, truthful surface where a reader can see what is real, where it came from and what has been left out.",
  },
