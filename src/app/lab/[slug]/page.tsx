...
const articles: Record<string, ArticleContent> = {
  ...
  "workflow-impact-preview": { lead: "An independent product exploration built on n8n: a pre-publish replay for the consequential moment when an automation changes, but its effect on prior cases is still unclear.", quote: "Automation should make consequences faster to inspect, not easier to miss.", sections: [{ title: "The risky moment is before publish", paragraphs: ["In n8n, a workflow can look valid on the canvas while a routing condition, tool permission or credential changes what happens to real work.", "This prototype asks a smaller product question around that publishing moment: which representative past executions would take another path?"] }, { title: "Compare outcomes, not node diagrams", paragraphs: ["The proposed n8n-style flow remains visible, but the emphasis shifts to behaviour. Each replay puts the current and proposed outcome side by side.", "A difference is named as a route change, a new external side effect or a failure, so review does not require parsing the whole graph first."] }, { title: "Keep publish separate from approval", paragraphs: ["A safe refinement can proceed. A new external action or insufficient permission asks for review instead.", "The interaction is backed by a published n8n workflow: Webhook → Validate scenario → Evaluate replay policy → Build execution receipt. It accepts an allowlisted synthetic scenario and returns a typed decision as JSON; no customer data is replayed and no customer workflow is read or changed."] }], close: "n8n helps people assemble and run automation. This impact preview explores how it could also help them understand what a change will do before it becomes somebody else’s problem." },
  "parallel-not-blind": {
