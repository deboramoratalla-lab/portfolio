...
const articles: Record<string, ArticleContent> = {
  ...
  "workflow-impact-preview": { lead: "An independent product exploration built on n8n: a pre-publish replay for understanding how a change to an agent’s system message, memory, model or tools would affect prior conversations.", quote: "An agent change is safe when its behaviour is understood—not merely when its nodes still connect.", sections: [{ title: "The risky moment is before publish", paragraphs: ["An n8n agent can look valid on the canvas while a small change to its system message, memory, model credential or tool instructions materially changes what it says and does.", "This prototype asks a focused question at that publishing moment: which representative conversations would end differently, and would the proposed agent gain any new external actions?"] }, { title: "Replay behaviour, not just the graph", paragraphs: ["The proposed agent flow keeps the important concepts visible: Chat Trigger, conversation memory, approved knowledge, model and system message, tool selection, human review and a traceable response.", "Each replay compares the current and proposed outcome. Differences are named as response changes, new tool calls, bypassed approval gates or failures, so a reviewer does not need to reverse-engineer the whole graph first."] }, { title: "Agentic execution needs deterministic guardrails", paragraphs: ["The system under review is an AI agent; the system issuing the safety verdict is deliberately not. New external writes require a human review, and a missing model credential blocks publishing.", "The interaction is backed by a published deterministic n8n workflow: Webhook → Validate scenario → Evaluate replay policy → Build execution receipt. It accepts an allowlisted synthetic scenario and returns a typed decision as JSON; no production agent, model, tool or customer data is called or changed."] }], close: "n8n gives builders the primitives to connect a trigger, model, memory and tools into an agent. This impact preview explores the missing confidence layer: understanding how that agent will behave before publishing it." },
  "parallel-not-blind": {
